...
def parse_line(line, vocab):
    def get_content(record):
        ...
        return [tokens, np.int32(fields[0])]  # [text_content, label]
...
def model_fn_builder(features, labels, mode, params):
    ...
    learning_rate = tf.compat.v1.train.exponential_decay(params['learning_rate'], tf.train.get_global_step(),
                                                         10000, 0.65, staircase=True)
...
def main(unused_argv):
    ...
    '''
    # train_and_evaluate() 用参数控制每N步训练就存储checkpoint并进行验证
    train_spec = tf.estimator.TrainSpec(
        input_fn=lambda: input_fn(path_train, path_words, FLAGS.shuffle_buffer_size, config["num_oov_buckets"]),
        max_steps=FLAGS.train_steps
    )

    eval_spec = tf.estimator.EvalSpec(input_fn=lambda: input_fn(path_eval, path_words, 0, config["num_oov_buckets"]),
                                      throttle_secs=300)
    ...
    eval_results = classifier.evaluate(
        input_fn=lambda: input_fn(path_eval, path_words, 0, config["num_oov_buckets"])
    )
